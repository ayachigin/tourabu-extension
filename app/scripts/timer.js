...
TourabuEx.Timer = (function () {
    ...
    Timer.prototype.init = function () {
        ...
        events.bind(events.SECOND_CHANGE, function () {
            if ((self.seconds++) % 5 === 0) {
                self.checkTimers();
            }
        });
    ...
    Timer.prototype.checkTimers = function () {
        ...
        for (i = l - 1; i >= 0; i--) {
            if (this.timers[i].end < now) {
                if (this.timers[i].type) { types.push(this.timers[i].type); }
                types.push('end');
    ...
    Timer.prototype.saveTimers = function () {
        ...
        for (i = 0; i < l; i++) {
            timersToSave[i] = {};
            for (var k in this.timers[i]) {
                if (this.timers[i].hasOwnProperty(k)) {
